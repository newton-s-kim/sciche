...
/// \note In Windows only one class is allowed. Using multiple figures are controlled by a figure number. In Linux we
/// may use one instance per figure.
///
class gplot {
private:
    ...
    std::string get_type(T x)
    {
        ...
        if (typeid(x) == typeid(int64_t))
            return "int64";
        if (typeid(x) == typeid(arma::sword))
            return "int64";
        if (typeid(x) == typeid(uint64_t))
            return "uint64";
        if (typeid(x) == typeid(arma::uword))
            return "uint64";
...
// #define term "aqua"
    ...
    void window(const int fig, const char* name, const int x, const int y, const int width, const int height)
    {
        fig_ix = fig;
        std::ostringstream tmp_s;
        tmp_s << "set term " << term << " " << fig << " title \"" << name << "\" position " << x << "," << y << " size "
              << width << "," << height;
        std::string s = tmp_s.str();
        send2gp(s.c_str());
        send2gp("reset");
    ...
    void fast_plot(const arma::Col<T>& x, const std::string fmt_args = "with lines")
    {
        ...
        s = "plot '-' binary format='%" + fmt + "' array=(" + std::to_string(x.n_elem) + ") " + fmt_args;
        send2gp(s.c_str());
    ...
    void image(const arma::Mat<T>& x)
    {
        ...
        s = "plot '-' binary array=(" + std::to_string(x.n_cols) + "," + std::to_string(x.n_rows) +
            ") scan=yx format='%" + fmt + "' w image";
        send2gp(s.c_str());
        std::fwrite(x.memptr(), sizeof(x.at(0)), x.n_elem, gnucmd);
        flush_cmd_buf();
    }
    ...
    void mesh(const arma::Mat<T>& x)
    {
        ...
        std::string fmt = get_type(x.at(0));
        std::string s = "splot '-' binary array=(" + std::to_string(x.n_cols) + "," + std::to_string(x.n_rows) +
                        ") scan=yx format='%" + fmt + "' w lines";
        send2gp(s.c_str());
        std::fwrite(x.memptr(), sizeof(x.at(0)), x.n_elem, gnucmd);
        flush_cmd_buf();
    }
    ...
    /// - gif
    ///
    /// \note When 'latex' output is used the '\' must be escaped by '\\\\' e.g set_xlabel("Frequency $\\\\omega = 2
    /// \\\\pi f$")
    ////////////////////////////////////////////////////////////////////////////////////////////
