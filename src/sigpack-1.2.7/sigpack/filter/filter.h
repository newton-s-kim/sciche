...
///
/// Implements FIR/MA filter functions as \f[  y(n) =
/// \sum_{k=0}^{M-1}{b_kx(n-k)}=b_0x(n)+b_1x(n-1)+...+b_{M-1}x(n-(M-1))\f] where M is the number of taps in the FIR
/// filter. The filter order is M-1. Adaptive update of filter is possible with LMS or NLMS algorithms
...
///
/// Implements IIR/ARMA filter functions as \f[  a_0y(n) =
/// b_0x(n)+b_1x(n-1)+...+b_{M-1}x(n-(M-1))-a_1y(n-1)-...-a_{M-1}y(n-(M-1))\f] where M is the number of taps in the FIR
/// filter part and M is the number of taps in the IIR filter. The filter order is (M-1,M-1)
///
template <class T1, class T2, class T3>
class IIR_filt {
private:
...
arma_inline arma::vec fd_filter(const arma::uword M, double fd)
{
    ...
    if (M % 2 == 1)
        fd = fd - 0.5; // Offset for odd nr of taps
    for (arma::uword m = 0; m < M; m++) {
