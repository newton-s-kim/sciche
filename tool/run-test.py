...
_executable = "build/sciche/sciche"
    ...
    if _failed == 0:
        print(
            "All {} tests passed ({} expectations).".format(
                term.green(_passed), _expectations
            )
        )
    else:
        print(
            "{} tests passed. {} tests failed.".format(
                term.green(_passed), term.red(_failed)
            )
        )
...
class Test:
        ...
        for line in lines:
            ...
            if match is not None:
                ...
                if language is not None and language == "c":
                    # loch works more like jlox then clox.
    ...
    def run(this):
        ...
        if len(this._option) > 0:
            result = subprocess.run(
                [_executable, this._option, this._path],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            )
        else:
            result = subprocess.run(
                [_executable, this._path],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            )
    ...
    def _validateRuntimeError(this, errorLines):
        if len(errorLines) < 2:
            this._fail(
                "Expected runtime error '{}' and got none.".format(
                    this._expectedRuntimeError
                )
            )
            return
        if errorLines[0] != this._expectedRuntimeError:
            this._fail(
                "Expected runtime error '{}' and got:".format(
                    this._expectedRuntimeError
                )
            )
    ...
    def _validateExitCode(this, exitCode, errorLines):
        ...
        if len(errorLines) > 10:
            ...
            this._fail(
                "Expected return code {} and got {}. Stderr:".format(
                    this._expectedExitCode, exitCode
                ),
                errorLines,
            )
    ...
    def _validateOutput(this, outputLines):
        ...
        for outputLine in outputLines:
            if index >= len(this._expectedOutputs):
                ...
                if expected.output != outputLine:
                    this._fail(
                        "Expected output '{}' on line {} and got '{}'.".format(
                            expected.output, expected.line, outputLine
                        )
                    )
            index += 1
        ...
        while index < len(this._expectedOutputs):
            expected = this._expectedOutputs[index]
            this._fail(
                "Missing expected output '{}' on line {}.".format(
...
def main():
    parser = argparse.ArgumentParser(prog=sys.argv[0], description="loch tester")
    parser.add_argument("--use_vm", action="store_true")
    parser.add_argument("suite", nargs="?", default="")
    ...
    _defineTestSuites()
    if len(args.suite) > 0:
        _runSuite(args.suite, option)
    else:
        _runSuite("loch", option)
