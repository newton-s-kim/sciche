...
namespace sce {
NativeObject::NativeObject(tsl::robin_map<std::string_view, NativeObjectBoundFn>& apis,
...
{
    tsl::robin_map<std::string_view, NativeObjectBoundFn>::iterator it = m_apis.find(name);
    if (it == m_apis.end())
...
{
    tsl::robin_map<std::string_view, NativeObjectBoundProperty>::iterator it = m_properties.find(name);
