...
namespace sce {
NativeClass::NativeClass(tsl::robin_map<std::string_view, NativeClassBoundFn>& apis,
...
{
    tsl::robin_map<std::string_view, NativeClassBoundFn>::iterator it = m_apis.find(name);
    if (it == m_apis.end())
...
{
    tsl::robin_map<std::string_view, NativeClassBoundProperty>::iterator it = m_constants.find(name);
